...
|--------------------------------------------------------------------------
| AI-powered chat interface for a specific document.
|
| Responsibilities:
| 1. Load previous chat history for the selected document.
| 2. Display user and AI messages.
| 3. Send user questions to the backend.
| 4. Receive AI responses generated using Vector RAG.
| 5. Automatically scroll to the latest message.
|--------------------------------------------------------------------------
*/
...
const ChatInterface = () => {
  ...
  /*
|--------------------------------------------------------------------------
| useRef() in ChatInterface
|--------------------------------------------------------------------------
| We create a reference to an empty div placed at the bottom of the chat.
| This allows us to directly access that DOM element without causing
| a re-render.
|
| Whenever the chat history changes (new message, AI reply, or initial
| history load), we call scrollIntoView() on this referenced div so the
...
*/
  ...
  // Scroll the chat window to the latest message whenever
  // a new message is added to the conversation.
  // Scroll the chat to the bottom by scrolling the referenced div
  // into the visible area. This keeps the latest message in view.
  ...
  // Whenever the conversation history changes
  // (initial load, user message, or AI response),
  ...
  // The backend performs Vector RAG, generates an AI response,
  // and the conversation history is updated.
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!message.trim()) return;

    //we created a userMessage object and added in history so that the user message appears on the chat till then the gemini api call is completed.
    const userMessage = {
      role: "user",
      content: message,
      timestamp: new Date(),
  ...
  // AI messages appear on the left with Markdown rendering.
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
        {/* if history.length === 0 then show a page of Start conversation */}
        {history.length === 0 ? (
    ...
    We scroll to this element whenever history updates,
    making the latest message automatically visible. */}
